...
public:
	...
	static bool cmpLength(const string& a, const string& b) { return a.size() < b.size(); }

	string sortingMethod(vector <string> stringList) {
		vector<string> lexico(stringList);
		vector<string> leng(stringList);
		sort(lexico.begin(), lexico.end());
		sort(leng.begin(), leng.end(), cmpLength);
		
		bool isLexico = lexico == stringList;
		bool isLeng = leng == stringList;
		if(isLexico && isLeng)
			return "both";
		if(isLexico)
